...
def greedy_mesh_maxrect(voxels: np.ndarray, size: tuple[int, int, int], palette_rgba: np.ndarray) -> dict[str, np.ndarray]:
    sx, sy, sz = size
    if voxels.shape != (sx, sy, sz):
        raise ValueError("voxels shape does not match size")
    ...
    for d in range(3):
        ...
        for x[d] in range(dims[d] + 1):
            # Build face mask (same semantics as greedy_mesh)
            for x[v] in range(dims[v]):
                for x[u] in range(dims[u]):
                    a = 0
                    b = 0
